...
def draw_point(img, point, bbox_size=10, color=(0, 0, 255)):
    img[point[1] - bbox_size // 2: point[1] + bbox_size // 2,
    point[0] - bbox_size // 2: point[0] + bbox_size // 2] = color
...
data_root = os.path.join(os.getenv("data_dir", "datasets"), "CUB_200_2011")
...
for bbox_line in bbox_lines:
    cts = bbox_line.split(' ')
    img_id, bbox_x, bbox_y, bbox_width, bbox_height = int(cts[0]), int(cts[1].split('.')[0]), int(
        cts[2].split('.')[0]), int(cts[3].split('.')[0]), int(cts[4].split('.')[0])
...
for part_loc_line in part_loc_lines:
    ...
    img_id, part_id, loc_x, loc_y, visible = int(content[0]), int(content[1]), int(float(content[2])), int(
        float(content[3])), int(content[4])
...
train_ids = []
CUB_PROCESSED_DIR = 'data/CUB/class_attr_data_10'
...
part_attributes_names = ['breast', 'forehead', 'nape', 'throat', 'eye', 'belly', 'back', 'leg', 'crown',
                         'beak']  # The names of part attributes
