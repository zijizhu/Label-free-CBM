...
from .local_parts import attributes_names, part_attributes_names, id_to_attributes, id_to_bbox, id_to_part_loc, \
...
class Cub2011Eval(Dataset):
    base_folder = os.path.join('cub200_cropped', 'test_cropped')

    def __init__(self, root="datasets", train=False, transform=None, loader=default_loader):
...
@torch.no_grad()
def evaluate_concept_trustworthiness(all_activation_maps: list[torch.Tensor],
                                     all_img_ids: list[int] | torch.Tensor,
                                     bbox_half_size=36,
                                     img_size=224):
    """
    ...
    for attri_idx in tqdm(range(n_attributes)):
        ...
        pred_bboxes = torch.stack([pred_y1, pred_y2, pred_x1, pred_x2], dim=1).to(device)  # (n_select_samples, 4)
        ...
        part_exist[cal_img_indexes]  # (n_cal_samples, 4), (n_cal_samples, Np, 2), (n_cal_samples, Np)
